...
// Verify that grpcServer implements the api.LogServer interface.
var _ api.LogServer = (*grpcServer)(nil)

const (
	// Matches the values in policy.csv
	objectWildcard = "*"
	...
	for {
		...
		default:
			...
			case api.ErrOffsetOutOfRange:
				// When the end of the log is reached, wait until new records are added
				// and then continue streaming logs to the client.
				continue
			default:
				return err
			}
...
func NewGRPCServer(config *Config, opts ...grpc.ServerOption) (*grpc.Server, error) {
	...
	opts = append(opts,
		...
		grpc.StatsHandler(&ocgrpc.ServerHandler{}), // For stats and traces.
	)
...
// gRPC interceptor (also called middleware).
// Reads the CN value from TLS info and adds it as "subject" into the context.
